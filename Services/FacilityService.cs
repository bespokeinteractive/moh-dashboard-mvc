...
namespace hrhdashboard.Services
{
    ...
    public class FacilityService
    {
        ...
        public List<Level> GetFacilityCategorizationByLevels(Ward ward)
        {
            ...
            SqlServerConnection conn = new SqlServerConnection();
            SqlDataReader dr = conn.SqlServerConnect("SELECT fl_idnt, ISNULL(fc_count,0) fl_count FROM Levels LEFT OUTER JOIN (SELECT fc_level, COUNT(*) fc_count FROM Facility WHERE fc_level<>99 AND fc_subcounty=" + ward.Id + " GROUP BY fc_level) AS Foo ON fc_level=fl_idnt WHERE fl_idnt<>99 ORDER BY fl_idnt");
            if (dr.HasRows)
            {
                while (dr.Read())
                {
